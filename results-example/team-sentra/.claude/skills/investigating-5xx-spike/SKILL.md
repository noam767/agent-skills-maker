...
description: Use this skill when the team reports a 5xx burst, error-rate spike, or HTTP 500 page — triggers include "we're seeing 5xx on service X", "errors spiking", "page on payments", "what's failing in prod". Drives the canonical Sentra workflow: Splunk saved search → ArgoCD recent-deploy correlation → rollback recommendation, then Jaeger exemplar trace if needed.
---
