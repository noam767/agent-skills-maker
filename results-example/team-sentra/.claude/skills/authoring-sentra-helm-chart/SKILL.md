...
description: Use this skill when a team Sentra engineer asks to create or scaffold a Helm chart for a new service — triggers include "new chart for X", "Helm chart for service Y", "add a service to sentra/charts", "scaffold a deployment". Produces a chart that uses the sentra-service library chart, pins images by digest, follows the per-env values layout, and passes the conftest policy gates.
---
