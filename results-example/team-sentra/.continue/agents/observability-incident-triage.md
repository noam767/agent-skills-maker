...
description: "Use proactively when a production alert fires or a customer-impact symptom is reported (latency spike, 5xx burst, OOMKilled wave). Walks the team Sentra observability stack (Grafana → Prometheus → Splunk → Jaeger) to pin down a root cause and propose an ArgoCD rollback when appropriate."
---
# Purpose
